...
def main():
	...
	parser.add_argument("-d", "--download_dir", type=str, required=True,
						help=help_text.format("download directory", directory))
	parser.add_argument("-i", "--input", type=str, required=True,
						help=help_text.format("input directory", directory))
	parser.add_argument("-o", "--output", type=str, default=directory,
						help=help_text.format("output directory", directory))
	a = parser.parse_args()
	...
	print("Get translated srt from translate-subtitles.com")
	url = 'https://translate-subtitles.com'
	browser = webdriver.Safari()
	browser.get(url)
	upload = browser.find_element_by_id('file')
	for fn in filenames:
		file = "{}/{}.srt".format(a.output, fn)
		upload.send_keys(file)
		sleep(15)
		# move the downloaded translated srt to the output directory
		shutil.copy("{}/{}.en.srt".format(a.download_dir, fn), a.output)
		os.remove("{}/{}.en.srt".format(a.download_dir, fn))
	# close the invoked browser
	browser.quit()

	print("Combining translated srt with the original srt.")
	# to combine the two srt files 
	srt_files = [f for f in os.listdir(a.output) if f.endswith('.srt')]
	srt_filenames = [os.path.splitext(x)[0] for x in srt_files]
	pairs = [(f1, f2) for f1 in srt_filenames for f2 in srt_filenames if f1 in f2]
	pairs = [(f1, f2) for f1, f2 in pairs if f1 != f2]
	srt_pairs = [(os.path.join(a.output, '{}.srt'.format(f1)), os.path.join(a.output, '{}.srt'.format(f2))) for f1, f2 in pairs]
	for f1, f2 in srt_pairs:
		srt.srtmerge([f1, f2], '{}.dual.srt'.format(os.path.splitext(f1)[0]))

	# to clean up all generated srt, keep only the dual.srt in the output directory
	EXT = SUPPORTED_EXTENSIONS + ['.srt']
	all_files = [os.path.join(a.output, fn) for fn in os.listdir(a.output)]
	all_files = [f for f in all_files for ext in EXT if f.endswith(ext)] # include only XML, VTT and SRT files
	result_srt = ["{}/{}.dual.srt".format(a.output, f) for f in filenames] # files to keep 
	to_remove = list(set(all_files) - set(result_srt)) # files that are not in result_srt 
	for rm in to_remove:
		os.remove(rm)

	print("Done! All .srt with dual langauage are in the output directory")
